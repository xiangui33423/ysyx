...
#define immB() do { *imm = (BITS(i, 31, 31) << 12) | (BITS(i, 6, 6) << 11) | (SEXT(BITS(i, 30, 25),6) << 5) | SEXT((i, 10, 8),4) << 1;} while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if(src1 != src2) s->dnpc = s->pc + imm + 4);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if(src1 == src2) s->dnpc = s->pc + imm + 4);
