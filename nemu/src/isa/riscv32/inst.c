...
#define immB() do { *imm = (BITS(i, 31, 31) << 11) | (BITS(i, 6, 6) << 10) | (SEXT(BITS(i, 30, 24),7) << 4) | BITS(i, 10, 7);} while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc += 2*(imm)-4);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = src1 + imm; R(rd) = s->pc + 4);
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , I, Mw(src1 + imm, 1, src2));
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if(src1 != src2) s->dnpc += (imm << 1) -4);
