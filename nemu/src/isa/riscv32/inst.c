...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if((src1) >= (src2)) s->dnpc = s->pc + (imm&0xfffffffe));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, int i,mask = BITS(src1,31,31); for(i = 0;i < imm;i++){ src1 = (src1 >> 1) | (mask << (31-i));}R(rd) = src1);
