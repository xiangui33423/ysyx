...
#define immB() do { *imm = (SEXT(BITS(i, 31, 31), 1) << 12) | (BITS(i, 7, 7) << 11) | (SEXT(BITS(i, 30, 25),6) << 5) | SEXT((i, 11, 8),4) << 1;} while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, printf("%d\n",(imm&0xfffffffe));if(src1 != src2) s->dnpc = s->pc + (imm&0xfffffffe));
