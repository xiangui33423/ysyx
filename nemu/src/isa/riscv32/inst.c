...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if(src1&0xfffffffe < src2&0xfffffffe) s->dnpc = s->pc + (imm&0xfffffffe));
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(rd) = src1 + src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(rd) = src1 - src2);
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, R(rd) = (src1&0xffffffff < src2&0xffffffff) ? 1 : 0);
