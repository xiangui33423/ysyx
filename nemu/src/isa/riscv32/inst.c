...
#define immB() do { *imm = (BITS(i, 31, 31) << 12) | (SEXT(BITS(i, 30, 25),6)) << 5 | (BITS(i, 7, 7) << 11)  | (BITS(i, 11, 8)) << 1;} while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, word_t res = Mr(src1 + imm, 2); if(BITS(res,15,15)) R(rd) = res | 0xffff0000; else R(rd) = res;);
  ...
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, if(src1 >= src2) s->dnpc = s->pc + (imm&0xfffffffe));
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, if(src1 < src2) s->dnpc = s->pc + (imm&0xfffffffe));
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if((int32_t)src1 < (int32_t)src2) s->dnpc = s->pc + (imm&0xfffffffe));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if(src1 == src2) s->dnpc = s->pc + (imm&0xfffffffe));
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = (uint64_t)src1 / (uint64_t)src2);
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(rd) = (uint64_t)src1 % (uint64_t) src2);
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = src1 * src2);
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, int64_t res = (int64_t)(int32_t)src1 * (int64_t)(int32_t)src2; R(rd) = BITS(res,63,32));
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divh   , R, R(rd) = (int32_t)src1 / (int32_t)src2);
