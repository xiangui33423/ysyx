...
#define immB() do { *imm = (BITS(i, 31, 31) << 12) | (SEXT(BITS(i, 30, 25),6)) << 5  | (SEXT((i, 11, 8),4)) << 1 | (BITS(i, 7, 7) << 11) ;} while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", bne    , B, if(src1 != src2) s->dnpc = s->pc + (imm&0xfffffffe));
